...
Within the UMIP is a sample coding point the [Google BigQuery Ethereum dataset](https://cloud.google.com/blog/products/data-analytics/ethereum-bigquery-public-dataset-smart-contract-analytics). Even though BigQuery is a leading analytical database and would have enabled the strongest degree of flexibility, like using preferred analytics tools like Tableau, BigQuery was not further evaluated. 
...
The Dashboard designer is a browser based tool to design and compose the dashboards. It provides basic design and layout options, but is sufficient enough to create visual appealing Dashboards. 
Compared to leading commercial business intelligence or dashboarding tools, the Dune designer provided about 10%-20% of their functionality. 
...
The designer allows to drag & drop textboxes and widgets on the dashboard. The widgets provide the option for layout formatting with a markup language and adding images. 
...
Next to the SQL-queries, the indexing/decoding for project specific smart-contracts and the abstractions layer are important services for creating blockchain related data queries. 
In case specific project related contracts are required for queries, the Dune team provides a [form]( https://duneanalytics.retool.com/embedded/public/892af55f-a6ff-41df-b203-f8acb6f0a38b) to schedule contracts for decoding. 
...
These views and tables are referred as [abstraction layer]( https://duneanalytics.retool.com/embedded/public/892af55f-a6ff-41df-b203-f8acb6f0a38b) within the Dune data platform. Building domain specific and predefined datasets is essential for providing an efficient and sustainable infrastructure for developers and within the Dune database- and data models. 

When implementing dashboards for new projects, the contracts have to be maintained in abstraction layer tables. For example contracts have to be added to the ER20 tokens table, to be made available for DEX queries. During implementation, the maintenance of new contracts and tokens within the abstraction layer will happen via GitHub workflows like Pull-requests. These interactions require basic knowledge of GitHub. 
...
The general setup here reflected to typical implementation of a dashboard project in corporate environments. The domain experts with strong DeFi background defined the requirements documents as  dashboard mock-up. 
...
* In case of teams with distributed skillsets the Solidity developers or DeFi experts ideally create specifications, similar to the UMA [UMIPs]( https://github.com/UMAprotocol/UMIPs/blob/master/UMIPs/umip-16.md), with pseudocode examples to be adapted in SQL 

* Ideally the Dune team and community would curate a glossary and quality assured query gallery for implementing common DeFi queries and charts like TVL, unique wallet holder, minting etc. Currently there are  almost too many examples and queries. A gallery with most frequent DeFi performance indicators will help developers to on-board quickly. This series of tweets is already a good starting point: 
